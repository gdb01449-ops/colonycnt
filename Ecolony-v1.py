...
class ColonyCounterApp:
    ...
    def _build_left_panel(self):
        ...
        btns = [
            ...
            ("프리셋 적용", self.apply_target_preset),
        ...
        settings_outer = tk.LabelFrame(self.left_panel, text="설정")
        ...
        row = 0
        tk.Label(self.settings_inner, text="분석 대상", anchor="w").grid(row=row, column=0, sticky="ew", padx=8, pady=6)
        target_menu = tk.OptionMenu(self.settings_inner, self.analysis_target, "fecal_coliform", "total_coliform", command=lambda _: self.apply_target_preset())
        target_menu.grid(row=row, column=1, sticky="ew", padx=8, pady=6)
        row += 1

        tk.Label(self.settings_inner, text="분석 방식", anchor="w").grid(row=row, column=0, sticky="ew", padx=8, pady=6)
        tk.OptionMenu(self.settings_inner, self.analysis_mode, "targeted", "general").grid(row=row, column=1, sticky="ew", padx=8, pady=6)
        row += 1

        common_controls = [
            ("최소 면적", self.min_area_var),
            ("최대 면적", self.max_area_var),
            ("원형도 최소", self.circularity_var),
            ("최소 중심거리", self.min_distance_var),
            ("가장자리 제외비율", self.edge_margin_ratio_var),
            ("최소 반지름", self.min_radius_var),
            ("최대 반지름", self.max_radius_var),
        ]
        for label_text, var in common_controls:
            tk.Label(self.settings_inner, text=label_text, anchor="w").grid(row=row, column=0, sticky="ew", padx=8, pady=6)
            tk.Entry(self.settings_inner, textvariable=var).grid(row=row, column=1, sticky="ew", padx=8, pady=6)
            row += 1

        tk.Label(self.settings_inner, text="--- 분원성대장균 전용 ---", anchor="w", fg="navy").grid(row=row, column=0, columnspan=2, sticky="ew", padx=8, pady=(10, 4))
        row += 1
        fecal_controls = [
            ("파란색 H 최소", self.fecal_h_min_var),
            ("파란색 H 최대", self.fecal_h_max_var),
            ("파란색 S 최소", self.fecal_s_min_var),
            ("파란색 V 최대", self.fecal_v_max_var),
            ("blackhat 임계값", self.fecal_blackhat_thresh_var),
        ]
        for label_text, var in fecal_controls:
            tk.Label(self.settings_inner, text=label_text, anchor="w").grid(row=row, column=0, sticky="ew", padx=8, pady=6)
            tk.Entry(self.settings_inner, textvariable=var).grid(row=row, column=1, sticky="ew", padx=8, pady=6)
            row += 1

        tk.Label(self.settings_inner, text="--- 총대장균 전용 ---", anchor="w", fg="darkred").grid(row=row, column=0, columnspan=2, sticky="ew", padx=8, pady=(10, 4))
        row += 1
        total_controls = [
            ("노랑 H 최소", self.total_yellow_h_min_var),
            ("노랑 H 최대", self.total_yellow_h_max_var),
            ("노랑 S 최소", self.total_yellow_s_min_var),
            ("노랑 V 최소", self.total_yellow_v_min_var),
            ("어두운 V 최대", self.total_dark_v_max_var),
            ("blackhat 임계값", self.total_blackhat_thresh_var),
        ]
        for label_text, var in total_controls:
            tk.Label(self.settings_inner, text=label_text, anchor="w").grid(row=row, column=0, sticky="ew", padx=8, pady=6)
            tk.Entry(self.settings_inner, textvariable=var).grid(row=row, column=1, sticky="ew", padx=8, pady=6)
            row += 1
    ...
    def apply_target_preset(self):
        ...
        self.info_label.config(text=f"프리셋 적용 완료\n- 분석 대상: {target}\n- 베이지 배지는 fecal_coliform\n- 붉은 배지는 total_coliform")
        ...
        try:
            ...
            self.info_label.config(
                text=(
                    ...
                    f"- 공통 필터: 면적 {self.min_area_var.get()}~{self.max_area_var.get()}, 원형도 ≥ {self.circularity_var.get():.2f}, 최소거리 ≥ {self.min_distance_var.get()}\n"
                    f"- 메모: {sample_note}\n"

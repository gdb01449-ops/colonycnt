...
class ColonyCounterApp:
    def __init__(self, root):
        self.root = root
        self.root.title("대장균 배지 자동 계수 프로그램 V5")
        ...
        self.analysis_target = tk.StringVar(value="분원성대장균")
        self.analysis_mode = tk.StringVar(value="전용분석")
        self.detection_level = tk.StringVar(value="기본")
    ...
    def _build_left_panel(self):
        ...
        for i, (text, cmd) in enumerate(btns):
            tk.Button(top_btn_frame, text=text, command=cmd, width=16).grid(
        ...
        tk.OptionMenu(
            ...
            "분원성대장균",
            "총대장균",
            command=lambda _: self.apply_target_preset(),
    ...
    def detect_plate_and_normalize(self, img_bgr):
        ...
        self.debug_images["1_정규화된_배지"] = plate_preview
    ...
    def detect_fecal_coliform(self, plate_bgr, plate_mask, plate_center_radius):
        ...
        self.debug_images["2_분원성_파란색마스크"] = cv2.cvtColor(blue_mask, cv2.COLOR_GRAY2BGR)
        self.debug_images["3_분원성_블랙햇"] = cv2.cvtColor(blackhat, cv2.COLOR_GRAY2BGR)
        self.debug_images["4_분원성_이진화"] = cv2.cvtColor(binary, cv2.COLOR_GRAY2BGR)
        return self.extract_candidates(plate_bgr, binary, plate_mask, plate_center_radius, mode_name="분원성")
    ...
    def detect_total_coliform(self, plate_bgr, plate_mask, plate_center_radius):
        ...
        self.debug_images["2_총대장균_노랑마스크"] = cv2.cvtColor(yellow_mask, cv2.COLOR_GRAY2BGR)
        self.debug_images["3_총대장균_블랙햇"] = cv2.cvtColor(blackhat, cv2.COLOR_GRAY2BGR)
        self.debug_images["4_총대장균_이진화"] = cv2.cvtColor(binary, cv2.COLOR_GRAY2BGR)
        return self.extract_candidates(plate_bgr, binary, plate_mask, plate_center_radius, mode_name="총대장균")

    def extract_candidates(self, plate_bgr, binary, plate_mask, plate_center_radius, mode_name="대상"):
        hsv = cv2.cvtColor(plate_bgr, cv2.COLOR_BGR2HSV)
        ...
        self.debug_images[f"5_{mode_name}_후보"] = candidate_debug
    ...
    def detect_general(self, plate_bgr, plate_mask, plate_center_radius):
        ...
        self.debug_images["2_일반_보정영상"] = cv2.cvtColor(corrected, cv2.COLOR_GRAY2BGR)
        self.debug_images["3_일반_이진화"] = cv2.cvtColor(binary, cv2.COLOR_GRAY2BGR)
        return self.extract_candidates(plate_bgr, binary, plate_mask, plate_center_radius, mode_name="일반")
    ...
    def draw_colonies(self, base_img, colonies, plate_center_radius):
        ...
        self.debug_images["9_최종결과"] = img.copy()
    ...
    def process_image(self):
        ...
        try:
            ...
            x, y, r = circle_info
            sample_note = "베이지 배지=분원성, 붉은 배지=총대장균" if self.analysis_mode.get() == "전용분석" else "일반 모드"
            self.info_label.config(
                text=(
                    f"V5 분석 완료\n"
                    f"- 분석 대상: {self.analysis_target.get()}\n"
    ...
    def show_debug_window(self):
        if not self.debug_window or not self.debug_window.winfo_exists():
            ...
            self.debug_frame.bind(
                "<Configure>",
                lambda e: self.debug_canvas.configure(scrollregion=self.debug_canvas.bbox("all")),
            )
            self.debug_canvas.configure(
                yscrollcommand=self.debug_scroll_y.set,
            ...
            self.debug_canvas.bind(
                "<Configure>",
                lambda e: self.debug_canvas.itemconfig(self.debug_window_id, width=e.width),
            )
        self.populate_debug_window()
    ...
    def save_csv(self):
        ...
        with open(path, "w", newline="", encoding="utf-8-sig") as f:
            ...
            for idx, c in enumerate(self.colonies, start=1):
                writer.writerow(
                    [
                        ...
                        c.get("cx", 0),
                        c.get("cy", 0),
                        round(c.get("area", 0.0), 2),
                        round(c.get("circularity", 0.0), 4),
                        "Y" if c.get("manual") else "N",
                        round(c.get("mean_h", 0.0), 2),
                        round(c.get("mean_s", 0.0), 2),
                        round(c.get("mean_v", 0.0), 2),
                        round(c.get("radius", 0.0), 2),
                    ]
                )
            writer.writerow([])
            ...
            writer.writerow(["검출 강도", self.detection_level.get()])
